...
import keras
import numpy
from utils.utils_RIFair import split_model_by_embedding_layer, false_fair
...
def FalseFairAdv(file1, files2, files3, files4, times, size1, size2, index):
    """
    进行准确公平性测试
    :return:
    """
    ...
    for i in range(label1.shape[0]):
        ...
        result = false_fair(models, label1[i], [index1[i], value1[i]], [index2, value2],
...
def FalseFairRetrain(file1, files2, files3, files4, times, size1, size2, index):
    """
    进行准确公平性测试
    :return:
    """
...
if __name__ == "__main__":
    ...
    age_result = ["../dataset/bank/adv/FF_{}_i.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/adv/FF_{}_s.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/adv/FF_{}_y.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/adv/FF_{}_t.txt".format("age_AutoInt_BL0")]
    FalseFairAdv(M_file, data_file, age_file, age_result, search_time, feature_size, embed_size, age)
    ...
    age_result = ["../dataset/bank/retrain/FF_{}_i.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/retrain/FF_{}_s.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/retrain/FF_{}_y.txt".format("age_AutoInt_BL0"),
                  "../dataset/bank/retrain/FF_{}_t.txt".format("age_AutoInt_BL0")]
    FalseFairRetrain(M_file, data_file, age_file, age_result, search_time, feature_size, embed_size, age)
