...
class HuggingFaceWrapper(torch.nn.Module):
    ...
    def predict_proba(self, texts):
        inumpyuts = self.tokenizer(texts, padding=True, truncation=True, max_length=128, return_tensors="pt").to(
            self.device)
        with torch.no_grad():
...
def generate_candidates_instances(original_tex_list, similar_text_list, perturbation_index, token, similarity_df,
                                  cosine_threshold=None, Seq_threshold=None):
    """
...
def get_best_TB_update(candidate_ori_texts, candidate_sim_texts, ori_label, candidate_tokens, model, current_J):
    ...
    if not candidate_ori_texts:
        return False, current_J, None, None, None, None
    ...
    for i, example_df in enumerate(attack_importance):
        try:
            ...
            for attack_step in range(steps_limit):
                ...
                c_ori, c_sim, c_tokens = generate_candidates_instances(ori_text, sim_text, idx_to_attack, target_word,
                                                                       attack_similarity, Seq_threshold=0.8)
                if len(c_ori) < 1:
                    continue

                # B. Find Best Candidate (The one that maximizes the Gap)
                TB_tag, new_J, p_ori, p_sim, ori_token, sim_token = get_best_TB_update(c_ori, c_sim, initial_label,
                                                                                       c_tokens, model, current_gap)
                ...
                if ori_token is not None:
                    ...
                    if TB_tag:
                        results.append({"original_example_id": i, "attack_step": attack_step, "final_gap": float(new_J),
                                        "final_text_ori": " ".join(ori_text), "final_text_sim": " ".join(sim_text),
                                        "probs_ori": p_ori, "probs_sim": p_sim})
                        print(f"true bias attack success {i}")
                        break
            results.append({"original_example_id": i, "attack_step": steps_limit, "final_gap": float(current_gap),
                            "final_text_ori": " ".join(ori_text), "final_text_sim": " ".join(sim_text),
                            "probs_ori": p_ori, "probs_sim": p_sim})
    ...
    for i, example_df in enumerate(attack_importance):
        try:
            ...
            for attack_step in range(steps_limit):
                ...
                # B. Find Best Candidate (The one that maximizes the Gap)
                TB_tag, new_J, p_ori, p_sim, ori_token, sim_token = get_best_FB_update(c_ori, c_sim, initial_label,
                                                                                       c_tokens, model, current_gap)
    ...
    for i, example_df in enumerate(attack_importance):
        try:
            ...
            for attack_step in range(steps_limit):
                ...
                # B. Find Best Candidate (The one that maximizes the Gap)
                FF_tag, new_J, p_ori, p_sim, ori_token, sim_token = get_best_FF_update(c_ori, c_sim, initial_label,
                                                                                       c_tokens, model, current_gap)
...
if __name__ == "__main__":
    ...
    for data_name in data_names:
        ...
        for model_key, model_path_raw in model_paths.items():
            ...
            output_file = f"{data_name}_data/{clean_model_name}_TB_attack_results_couple.pkl"
            # Load Model
            try:
            ...
            run_TB_attack_pipeline(model=model, importance_path=importance_file, attack_path=attack_file,
                                   output_path=output_file, attack_similarity=similarity_table, top_k=20)
    ...
    for data_name in data_names:
        ...
        for model_key, model_path_raw in model_paths.items():
            ...
            output_file = f"{data_name}_data/{clean_model_name}_FB_attack_results_couple.pkl"
            # Load Model
            try:
                print(f"--- Loading Model: {clean_model_name} ---")
                model = HuggingFaceWrapper(model_file, device)
                ...
                continue
            # Run the Pipeline Method
            run_FB_attack_pipeline(model=model, importance_path=importance_file, attack_path=attack_file,
                                   output_path=output_file, attack_similarity=similarity_table, top_k=20)
    ...
    for data_name in data_names:
        ...
        for model_key, model_path_raw in model_paths.items():
            ...
            output_file = f"{data_name}_data/{clean_model_name}_FF_attack_results_couple.pkl"
            # Load Model
            try:
                print(f"--- Loading Model: {clean_model_name} ---")
                model = HuggingFaceWrapper(model_file, device)
                ...
                continue
            # Run the Pipeline Method
            run_FF_attack_pipeline(model=model, importance_path=importance_file, attack_path=attack_file,
                                   output_path=output_file, attack_similarity=similarity_table, top_k=20)
