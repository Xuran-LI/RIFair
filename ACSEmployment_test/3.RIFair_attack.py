...
def optimize_discrete_attribute(dl_dx, dir_unit, x, attr_id):
    ...
    返回 adv_token_id
    """
    dot_products = [numpy.dot(dl_dx[attr_id], dir_unit[x[attr_id]][j]) for j in range(len(dir_unit[x[attr_id]]))]
    adv_token_id = int(numpy.argmax(dot_products))
    return adv_token_id


def false_bias(model, x_i1, x_v1, y, x_i2, x_v2, times, dir_unit):
    """
    ...
    for t in range(times):
        ...
        if has_discrete:
            ...
            grad_emb2 = compute_embedding_gradients(model, (adv_xi2, adv_xv2, not_y))

            dl_dx_emb = {}
            if isinstance(model, WideDeep):
                dl_dx_emb = {k: grad_emb1[k] + grad_emb2[k] for k in grad_emb1}
            elif isinstance(model, DeepFM):
                dl_dx_emb = {"second_order_emb": grad_emb1["second_order_emb"] + grad_emb2["second_order_emb"]}
            elif isinstance(model, AutoInt):
                dl_dx_emb = {k: grad_emb1[k] + grad_emb2[k] for k in grad_emb1}
            else:
                dl_dx_emb = None
        ...
        if has_discrete:
            ...
            _, grad2 = compute_input_gradients(model, (adv_xi2, adv_xv2, not_y))
            none_protect = adv_xi1 == adv_xi2
        else:
            _, grad1 = compute_input_gradients(model, (adv_xv1, y))
            _, grad2 = compute_input_gradients(model, (adv_xv2, not_y))
            none_protect = adv_xv1 == adv_xv2

        dl_dx_val = grad1 + grad2

        # ------------ 选特征：离散 + 连续 --------------------------
        if has_discrete:
            disc_id, cont_id = None, None
            for k in dl_dx_emb:
                disc_id, cont_id = select_max_gradient_feature(dl_dx_emb[k][0], dl_dx_val, discrete_mask,
                                                               continuous_mask, none_protect)
        else:
            disc_id, cont_id = select_max_gradient_feature(None, dl_dx_val, discrete_mask, continuous_mask,
                                                           none_protect)
...
def false_bias_attack(model, model_name, encode_tag, data_tag):
    """
    进行准确公平性测试，并保存攻击结果
    """
    ...
    for i in range(int(select_rate*index1.shape[0])):
        ...
        if encode_tag == "E":
            ...
            adv1, adv2, y, history = false_bias(model=model, x_i1=clean_xi, x_v1=clean_xv, y=clean_y,
                                                x_i2=aug_xi, x_v2=aug_xv, times=10, dir_unit=dir_unit)
        else:
            clean1 = [clean_xi * clean_xv]
            clean2 = [aug_xi * aug_xv]
            adv1, adv2, y, history = false_bias(model=model, x_i1=None, x_v1=clean_xi * clean_xv, y=clean_y,
    ...
    # ----------------------- 5. 保存到文件 -----------------------
    save_path = f"../dataset/ACS/employment/adv/FB_{data_tag}_{model_name}.npy"
    numpy.save(save_path, results, allow_pickle=True)
    print(f"[✓] false-bias attack finished. Results saved to {save_path}")
    return results


def true_bias(model, x_i1, x_v1, y, x_i2, x_v2, times, dir_unit):
    """
    生成 false-bias 对抗样本（连续 + 离散）
    """
    ...
    for t in range(times):
        ...
        # ------------ 计算 embedding 梯度（离散部分） ------------
        if has_discrete:
            grad_emb1 = compute_embedding_gradients(model, (adv_xi1, adv_xv1, not_y))
        ...
        # ------------ 连续梯度 -----------------------------------
        if has_discrete:
            _, grad1 = compute_input_gradients(model, (adv_xi1, adv_xv1, not_y))
            _, grad2 = compute_input_gradients(model, (adv_xi2, adv_xv2, y))
            none_protect = adv_xi1 == adv_xi2
        else:
            _, grad1 = compute_input_gradients(model, (adv_xv1, not_y))
...
def true_bias_attack(model, model_name, encode_tag, data_tag):
    """
    进行准确公平性测试，并保存攻击结果
    """
    ...
    for i in range(int(select_rate * index1.shape[0])):
        ...
        if encode_tag == "E":
            clean1 = [clean_xi, clean_xv]
            clean2 = [aug_xi, aug_xv]
            adv1, adv2, y, history = true_bias(model=model, x_i1=clean_xi, x_v1=clean_xv, y=clean_y,
                                               x_i2=aug_xi, x_v2=aug_xv, times=10, dir_unit=dir_unit)
        else:
            clean1 = [clean_xi * clean_xv]
            clean2 = [aug_xi * aug_xv]
            adv1, adv2, y, history = true_bias(model=model, x_i1=None, x_v1=clean_xi * clean_xv, y=clean_y,
                                               x_i2=None, x_v2=aug_xi * aug_xv, times=10, dir_unit=dir_unit)
    ...
    # ----------------------- 5. 保存到文件 -----------------------
    save_path = f"../dataset/ACS/employment/adv/TB_{data_tag}_{model_name}.npy"
    numpy.save(save_path, results, allow_pickle=True)
    print(f"[✓] false-bias attack finished. Results saved to {save_path}")
    return results


def false_fair(model, x_i1, x_v1, y, x_i2, x_v2, times, dir_unit):
    """
    生成 false-bias 对抗样本（连续 + 离散）
    """
...
def false_fair_attack(model, model_name, encode_tag, data_tag):
    """
    进行准确公平性测试，并保存攻击结果
    """
    ...
    for i in range(int(select_rate * index1.shape[0])):
        ...
        if encode_tag == "E":
            clean1 = [clean_xi, clean_xv]
            clean2 = [aug_xi, aug_xv]
            adv1, adv2, y, history = false_fair(model=model, x_i1=clean_xi, x_v1=clean_xv, y=clean_y,
                                                x_i2=aug_xi, x_v2=aug_xv, times=10, dir_unit=dir_unit)
        else:
            clean1 = [clean_xi * clean_xv]
            clean2 = [aug_xi * aug_xv]
            adv1, adv2, y, history = false_fair(model=model, x_i1=None, x_v1=clean_xi * clean_xv, y=clean_y,
                                                x_i2=None, x_v2=aug_xi * aug_xv, times=10, dir_unit=dir_unit)
    ...
    # ----------------------- 5. 保存到文件 -----------------------
    save_path = f"../dataset/ACS/employment/adv/FF_{data_tag}_{model_name}.npy"
    numpy.save(save_path, results, allow_pickle=True)
    print(f"[✓] false-bias attack finished. Results saved to {save_path}")
    return results
...
if __name__ == '__main__':
    ...
    for data_tag in data_tags:
        ...
        false_bias_attack(mlp_model, "MLP", encode_tag="O", data_tag=data_tag)
        true_bias_attack(mlp_model, "MLP", encode_tag="O", data_tag=data_tag)
        false_fair_attack(mlp_model, "MLP", encode_tag="O", data_tag=data_tag)
        print("MLP")
        ...
        false_bias_attack(wd_model, "WideDeep", encode_tag="E", data_tag=data_tag)
        true_bias_attack(wd_model, "WideDeep", encode_tag="E", data_tag=data_tag)
        false_fair_attack(wd_model, "WideDeep", encode_tag="E", data_tag=data_tag)
        print("WideDeep")
        ...
        false_bias_attack(dfm_model, "DeepFM", encode_tag="E", data_tag=data_tag)
        true_bias_attack(dfm_model, "DeepFM", encode_tag="E", data_tag=data_tag)
        false_fair_attack(dfm_model, "DeepFM", encode_tag="E", data_tag=data_tag)
        print("DeepFM")

        # AutoInt
        ai_path = "../dataset/ACS/employment/model/AutoInt.pt"
        ai_model = load_pytorch_model(AutoInt, ai_path, init_args={})
        false_bias_attack(ai_model, "AutoInt", encode_tag="E", data_tag=data_tag)
        true_bias_attack(ai_model, "AutoInt", encode_tag="E", data_tag=data_tag)
        false_fair_attack(ai_model, "AutoInt", encode_tag="E", data_tag=data_tag)
        print("AutoInt")
        ...
        false_bias_attack(tt_model, "TabTransformer", encode_tag="O", data_tag=data_tag)
        true_bias_attack(tt_model, "TabTransformer", encode_tag="O", data_tag=data_tag)
        false_fair_attack(tt_model, "TabTransformer", encode_tag="O", data_tag=data_tag)
