...
def select_max_gradient_feature(grad_emb, grad_val, discrete_mask, continuous_mask, none_protect_mask, same_impact):
    """
    选择梯度最大的离散、连续属性进行扰动
    """
    ...
    # 离散属性可扰动
    discrete_idx = numpy.where(discrete_mask & none_protect_mask & same_impact)[0]
    continuous_idx = numpy.where(continuous_mask & none_protect_mask & same_impact)[0]
...
def bias(model, x_i1, x_v1, y, x_i2, x_v2, times, dir_unit):
    """
    ...
    for t in range(times):
        ...
        if has_discrete:
            ...
            if isinstance(model, WideDeep):
                for k in grad_emb1:
                    same_impact1 = (numpy.sign(grad_emb1[k].cpu().numpy().sum(axis=2)) == numpy.sign(
                        grad_emb2[k].cpu().numpy().sum(axis=2)))[0]
                ...
                dl_dx_emb = None
                same_impact1 = numpy.zeros_like(discrete_mask, dtype=bool)
        ...
        same_impact2 = (numpy.sign(grad1) == numpy.sign(grad2))[0]
...
def bias_attack(model, model_name, encode_tag, data_tag):
    """
    进行准确公平性测试，并保存攻击结果
    """
    ...
    for i in range(int(select_rate * index1.shape[0])):
        ...
        if encode_tag == "E":
            ...
            adv1, adv2, y, history = bias(model=model, x_i1=clean_xi, x_v1=clean_xv, y=clean_y,
                                          x_i2=aug_xi, x_v2=aug_xv, times=10, dir_unit=dir_unit)
        else:
            clean1 = [clean_xi * clean_xv]
            clean2 = [aug_xi * aug_xv]
            adv1, adv2, y, history = bias(model=model, x_i1=None, x_v1=clean_xi * clean_xv, y=clean_y,
    ...
    # ----------------------- 5. 保存到文件 -----------------------
    save_path = f"../dataset/ACS/employment/adv/Bias_{data_tag}_{model_name}.npy"
...
if __name__ == '__main__':
    ...
    for data_tag in data_tags:
        ...
        bias_attack(mlp_model, "MLP", encode_tag="O", data_tag=data_tag)
        print("MLP")
        ...
        bias_attack(wd_model, "WideDeep", encode_tag="E", data_tag=data_tag)
        print("WideDeep")
        ...
        bias_attack(dfm_model, "DeepFM", encode_tag="E", data_tag=data_tag)
        print("DeepFM")

        # AutoInt
        ai_path = "../dataset/ACS/employment/model/AutoInt.pt"
        ai_model = load_pytorch_model(AutoInt, ai_path, init_args={})
        bias_attack(ai_model, "AutoInt", encode_tag="E", data_tag=data_tag)
        print("AutoInt")
        ...
        bias_attack(tt_model, "TabTransformer", encode_tag="O", data_tag=data_tag)
