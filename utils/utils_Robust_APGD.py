...
def APGD_generation(model, global_seeds, attack_eps, attack_step, proj_eps):
    """
    ...
    for i in range(x.shape[0]):
        ...
        perturbation = eps * numpy.sign(grad_0)
        x_1 = x_0 + perturbation
        perturbation = project(x_1 - x_0, proj_eps)
        x_1 = x_0 + perturbation

        pre_index = int(y_0[0])
        if model.predict(x_0)[0][pre_index] > model.predict(x_1)[0][pre_index]:
            f_max = model.predict(x_0)[0][pre_index]
            x_max = x_0
        else:
            f_max = model.predict(x_1)[0][pre_index]
            x_max = x_1

        for k in range(attack_step):
            # 损失函数梯度，扰动变量
            grad_1 = compute_loss_grad(x_1, y_cate_0, model)
            perturbation = eps * numpy.sign(grad_1)
            z_2 = x_1 + perturbation
            perturbation = project(z_2 - x_0, proj_eps)
            z_2 = x_0 + perturbation

            x_2 = x_1 + 0.75 * (z_2 - x_1) + 0.25 * (x_1 - x_0)
            perturbation = project(x_2 - x_0, proj_eps)
            x_2 = x_0 + perturbation

            list_f.append(model.predict(x_2)[0][pre_index])
            # 检查扰动后样本预测结果是否准确
            pre = numpy.argmax(model.predict(x_1), axis=1).reshape(-1, 1)[0]
            if pre != y_0:
                break

            if model.predict(x_2)[0][pre_index] > f_max:
                f_max = model.predict(x_2)[0][pre_index]
                x_max = x_2

            if k in var_w:
                list_f_max.append(model.predict(x_max)[0][pre_index])
                list_eps.append(eps)
                if k == 0:
                    continue
                elif APGD_Cond1(list_f, var_w[var_w.index(k) - 1], k) or APGD_Cond2(list_eps, list_f_max, -2, -1):
                    eps = 0.5 * eps
                    x_2 = x_max
...
def APGD_Cond1(list_f, min_index, max_index):
    """
    APGD_condition1
    :return:
    """
...
def APGD_Cond2(list_eps, list_f_max, min_index, max_index):
    """
    APGD_condition2
    :return:
    """
...
def run_APGD_experiment(model_file, test_file, attack_eps, attack_step, proj_eps):
    """
    PGD test generation
    :return:
    """
    ...
    adversarial_data = APGD_generation(model, test_data, attack_eps, attack_step, proj_eps)
    return adversarial_data


def robust_APGD_experiment(M_file, T_file, A_eps, A_step, P_eps, adversarial_data_file):
    """
    pgd 测试样本生成
    :return:
    """
    adversarial_data = run_APGD_experiment(M_file, T_file, A_eps, A_step, P_eps)
