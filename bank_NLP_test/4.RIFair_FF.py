...
import keras
import numpy
from utils.utils_RIFair import split_NLP_model_by_embedding_layer, NLP_false_fair, \
...
def FalseFairAdv(file1, files2, files3, files4, times, size1, size2):
    """
    进行准确公平性测试
    :return:
    """
    ...
    for i in range(test_label.shape[0]):
        ...
        result = NLP_false_fair(models, test_label[i], test_data[i], sim_test, times, delts, protect)
        # 保存搜索结果
...
def FalseFairRetrain(file1, files2, files3, files4, times, size1, size2):
    """
    进行准确公平性测试
    :return:
    """
    ...
    for i in range(train_label.shape[0]):
        ...
        result = NLP_false_fair(models, train_label[i], train_data[i], sim_test, times, delts, protect)
        # 保存搜索结果
        write_search_NLP_instances(adv_data_file, result[0])
        write_search_NLP_instances(adv_sim_file, result[1])
        write_search_labels(adv_label_file, result[2])
...
if __name__ == "__main__":
    ...
    age_result = ["../dataset/NLP/bank/adv/FF_{}_d.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/adv/FF_{}_s.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/adv/FF_{}_y.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/adv/FF_{}_t.txt".format("age_Transformer_BL0")]
    FalseFairAdv(M_file, data_file, age_file, age_result, search_time, feature_size, embed_size)
    ...
    age_result = ["../dataset/NLP/bank/retrain/FF_{}_d.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/retrain/FF_{}_s.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/retrain/FF_{}_y.txt".format("age_Transformer_BL0"),
                  "../dataset/NLP/bank/retrain/FF_{}_t.txt".format("age_Transformer_BL0")]
    FalseFairRetrain(M_file, data_file, age_file, age_result, search_time, feature_size, embed_size)
